...
function generateCardInHand() {
  let namaKartu = 'KHWS'
  let isiKartu = '23456789JQKA'
  let hasil = [];
  for(let i = 0;i < isiKartu.length;i++){
    let isi = []
    for(let j = 0;j < namaKartu.length;j++){
      let kata = isiKartu[i]+namaKartu[j]
      isi.push(kata)
    }//console.log(isi)
    hasil.push(isi)
  }
  let bagiKartu = []
  //let kartu = []
  let depan = Math.floor(Math.random() * hasil.length);
  let belakang = Math.floor(Math.random() * 3);
  bagiKartu.push(hasil[depan][belakang]);
  belakang = Math.floor(Math.random() * 3);
  bagiKartu.push(hasil[depan][belakang]);
  depan = Math.floor(Math.random() * hasil.length);
  bagiKartu.push(hasil[depan][belakang]);
  belakang = Math.floor(Math.random() * 3);
  bagiKartu.push(hasil[depan][belakang]);
  depan = Math.floor(Math.random() * hasil.length);
